...
# Mohn rho SSB
out_t6 = retro_df %>% filter(peel == 'Combined', type == 'SSB') %>% select(modname, Rho) %>%
         rename('Rho_ssb' = 'Rho')
out_t6 = out_t6 %>% mutate(Rho_ssb = round(Rho_ssb, digits = 2))
out_df = left_join(x = out_df, y = out_t6, by = 'modname')

# Mohn rho F
out_t6 = retro_df %>% filter(peel == 'Combined', type == 'F') %>% select(modname, Rho) %>%
          rename('Rho_f' = 'Rho')
...
<!-- ```{r} -->
<!-- #| out-width: "90%" -->
<!-- knitr::include_graphics(file.path(output_folder, "profile_plot_likelihood_NatM.png")) -->
<!-- ``` -->